...
-- 6) app_settings: tiny single-row config table so the admin page's
--    "show example landmarks on the globe" toggle can be seen live by
--    the monitor page, even when they're open on two different devices.
--    Purely a display switch — never touches research_responses/board_events.
create table if not exists app_settings (
  id       smallint primary key default 1,
  demo_on  boolean not null default false,
  constraint app_settings_singleton check (id = 1)
);
insert into app_settings (id, demo_on) values (1, false) on conflict (id) do nothing;
